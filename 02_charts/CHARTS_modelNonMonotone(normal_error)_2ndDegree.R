...
## quantile regression
taus <- seq(.01, .99, .01)
x1_x2 = poly(x = datset$x, d = 2) #2nd degree regressor
quantreg <- rq(formula = y ~ x1 + x2,
               data = data.frame(y=datset$y,
                                 x1=x1_x2[, 1],
                                 x2=x1_x2[, 2]),
                                 tau = taus)
...
x1_x2 = poly(x = datset$x, d = 2) #2nd degree regressor
ds <- data.frame(y = datset$y,
                 x1 = x1_x2[,1],
                 x2 = x1_x2[,2])
names(ds)[1] <- "resp"
datp <- data.frame(x1 = x1_x2[,1],
                   x2 = x1_x2[,2]) ### sample to predict
# formula
...
max.y <- max(datset$y)
sd.y <- sd(datset$y)
x_axis <- seq(floor(min.x+0.5), ceiling(max.x-0.5), round(sd.x, 0))
...
sd.y <- sd(quantiles_y)
y_axis <- seq(floor(min.y+0.5), ceiling(max.y-0.5), round(sd.y, 0))
